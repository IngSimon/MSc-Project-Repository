#Generates simulated data of a faraday thin source
#A value of rm=50 has been used.
#Script outputs six files.
...
def get_1d_data(input_dir):
        ...
        lam2 = np.loadtxt(lam_file, comments ='#')
...
stokesQ = p0*(nu/nu0)**(-alpha)*np.cos(2.*(rm*lam**2 + phi0))

stokesU = p0*(nu/nu0)**(-alpha)*np.sin(2.*(rm*lam**2 + phi0))
...
pl.title('Stokes parameters against $\lambda^2 [m^2]$')
...
np.savetxt("./data/wavelengths.txt",lam2)
...
#save the initial farady depth spectra

np.save("./data/fd_spec_initial.npy",fd_spec)
...
pl.xlabel('$\phi$ [rad $m^{-2}$]')   # string must be enclosed with quotes '  '
