...
###No wavelength extension

###Adapted from mlscript12.py
 

###Using linestyle='none'
...
def predict_Q_data(input_dir_2, filename4 = "training.txt"):
       ...
       x = np.linspace(np.min(lam_squared), np.max(lam_squared), 1024)
       ...
       #x = np.linspace(np.min(lam_squared), np.max(lam_squared), 5120)
       x = np.linspace(np.min(lam_squared), np.max(lam_squared), 1024)
