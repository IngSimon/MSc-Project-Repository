###An improved version of rmsynth2.py

###Script for performaing RM synthesis after data predicting missing data

###Data generated by the data imputation script (using either george or celerite methods)

###Compare difference between initial faraday depth spectra and final faraday depth spectra(after imputation)
 
###Script suitable for both faraday thick and faraday thin dataset

###Specify the size of the dataset

###script for use with celerite 
...
def get_1d_data(input_dir):
        ...
        #nu_file = input_dir + "frequencies.txt" #use original frequency values
        ...
        #nu = np.loadtxt(nu_file, comments="#") #use original frequency values
        ...
        #Generate frequency values that correspond to evenly space wavelengths
        nu = np.sqrt(9e16/lam2)
...
pl.plot(lam2,stokesQ, color = 'blue') #linestyle='none', marker='.', color = 'blue')

pl.plot(lam2,stokesU, color = 'orange') #linestyle='none', marker='.', color = 'darkorange')
...
fd_spec_final = fd_spec[::-1] ###added to reverse the appearance of the plot
...
pl.plot(phi,np.abs(fd_spec_final))

pl.xlabel('$\phi$ [rad $m^{-2}$]')

pl.ylabel('F ($\phi$)')

pl.axis([-1000,1000,-0.1,2.0])
...
#Get the difference between the intial and final faraday depth spectra
...
pl.plot(phi,np.abs(fd_spec_final), 'k--') #Initial faraday spectra

pl.plot(phi,np.abs(fd_spec_initial), color = 'k') #Faraday spectra after data imputation
